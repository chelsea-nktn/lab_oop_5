...
class FileDetails
{
    static void Main(string[] args)
    {
        try
        {
            int i;
            string fileName;
            try //Проверка на наличие аргументов
            {
                if (args.Length == 0)
                {
                    throw new Exception("NO ARGUMENTS"); // Если аргументов нет - исключение
                }
            } catch (Exception e)
            {
                Console.WriteLine("ERROR: {0}", e.Message); // Вывод сообщения об исключении
            }
            fileName = args[0]; //Присвоение fileName имени файла
            try // Проверка на исключения в ходе основной работы программы
            {
    ...
    static void Summarize(char[] symbols) //Объявление метода Summarize, аргументом которого является массив символов
        {
            try // Проверка на исключения в ходе работы функции Summarize
            {
            ...
            {
                Console.WriteLine("ERROR WHILE CHECKING FILE: {0}", e.Message);  // Вывод сообщения об исключении в ходе работы функции Summarize
            }
        }
    } 
}
